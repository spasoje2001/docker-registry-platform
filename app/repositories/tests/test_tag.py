...
class TagModelTests(TestCase):
    ...
    def test_list_tags_for_repository(self, MockService):
        ...
        tag1 = Tag.objects.create(
            repository=repo, name="v1.0.0", digest="sha256:" + "a" * 64, size=100000000
        )
        tag2 = Tag.objects.create(
            repository=repo, name="latest", digest="sha256:" + "b" * 64, size=105000000
        )
        tag3 = Tag.objects.create(
            repository=repo, name="dev", digest="sha256:" + "c" * 64, size=110000000
        )
    ...
    def test_list_tags_mock_registry(self, MockService):
        ...
        tag1 = Tag.objects.create(
            name="v1.0", repository=self.repo, digest="sha256:abc123", size=1024
        )
        tag2 = Tag.objects.create(
            name="v2.0", repository=self.repo, digest="sha256:def456", size=2048
        )
        tag3 = Tag.objects.create(
            name="latest", repository=self.repo, digest="sha256:ghi789", size=3072
        )
        ...
        url = reverse(
            "repositories:detail",
            kwargs={"owner_username": "user1", "name": "test-repo"},
        )
        response = self.client.get(url)
...
class OfficialRepoTagTests(TestCase):
    ...
    def setUp(self):
        ...
        self.admin2 = User.objects.create_user(
            username="admin2", password="testpass123", role=User.Role.ADMIN
        )
    ...
    def test_admin_can_create_tag_for_official_repo(self):
        ...
        url = reverse("repositories:tag_create_official", kwargs={"name": "python"})

        response = self.client.post(
            url, {"name": "3.12", "digest": "sha256:" + "a" * 64, "size": 100000000}
        )
...
class SyncTagsCommandTest(TestCase):
    def setUp(self):
        ...
        self.repository = Repository.objects.create(
            name="test-repo", description="Test repository", owner=self.user1
        )
    ...
    def test_sync_all_repositories(self, mock_service_class):
        ...
        stats = SyncStats(
            repos_processed=1, tags_created=3, tags_updated=1, tags_deleted=0
        )
...
class TagSearchSortTest(TestCase):
    ...
    def test_clean_button_preserves_explore_params(self):
        ...
        response = self.client.get(url_with_explore)
        self.assertContains(response, "q=search-val")
